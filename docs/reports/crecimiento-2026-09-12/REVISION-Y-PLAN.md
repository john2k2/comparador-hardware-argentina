...
Oportunidades para comercios muestra **671 productos detectados** y propone configurar Merchant Center para la pestaña Shopping. Son productos descubiertos, no anuncios pagados, ventas ni inventario propio. Como la compra se completa en otras tiendas, Merchant Center no es una prioridad del modelo actual; no se inició Merchant Center ni campañas publicitarias.

### Corrección de datos estructurados y decisiones pendientes — 25/09/2026

Se ajustó `buildProductJsonLd` para publicar `Product` solo cuando existe al menos una oferta comparable, reciente, disponible y con precio y URL válidos. Las fichas sin esas ofertas conservan `BreadcrumbList` y `Organization`, sin simular stock, valoración ni reseña. La prueba de regresión cubre ofertas inválidas, antiguas y agotadas. En el mismo constructor, un SKU procedente de las especificaciones se omite si contiene espacios Unicode; no se altera ni se reemplaza por el ID interno. Los tests focalizados, lint, TypeScript y build local pasaron. La desaparición de los tres errores de Search Console requiere publicación, rastreo posterior y nueva comprobación; los conteos del 23/09 siguen siendo el último dato observado.

La reseña editorial es independiente del stock, pero no hay reseñas verificadas y visibles por producto en las fichas actuales. Se puede crear una sección de análisis con autor, fecha, criterio y experiencia o fuentes identificables; solo después corresponde marcar `review` en esa misma ficha. La descripción automática, el consejo genérico y la ausencia de ofertas no son una reseña. `aggregateRating` requiere valoraciones reales y un conteo demostrable; sus advertencias siguen siendo opcionales.

El aviso de `license` del dataset sigue abierto como decisión sobre derechos de reutilización: primero documentar procedencia y alcance permitido del CSV, luego publicar términos visibles y reflejar la URL real en JSON-LD. No se elige una licencia abierta por defecto. Para Merchant, no se añadirán `shippingDetails` ni `hasMerchantReturnPolicy` del comparador mientras las compras y políticas pertenezcan a las tiendas externas. El SKU solo se revisará con datos verificables de producto o tienda. Merchant Center y anuncios se mantienen en evaluación comercial, sin tratar los 671 productos detectados como inventario propio.

### Modelo comercial confirmado por Jonathan — 25/09/2026

Comparador Hardware Argentina **no vende hardware directamente ni recibe reseñas de compradores propios**. Publica ofertas de comercios externos, dirige a sus tiendas y ofrece ayuda, revisión de compatibilidad y asesoría. La monetización inicial se orienta a servicios propios y espacios patrocinados identificados, sin alterar el orden orgánico ni atribuirse ventas o políticas de terceros. La futura venta o armado de PCs sería una línea distinta que requiere definir mercado, operación y condiciones antes de anunciarla como disponible.

Consecuencias para G17: conservar `Product` con `AggregateOffer` únicamente cuando represente ofertas verificables de tiendas y atribuir cada `seller` a la tienda real; no generar `review` ni `aggregateRating` a partir de fichas scrapeadas o de una relación comercial con la tienda. Las advertencias de Merchant sobre envíos y devoluciones no se resuelven inventando una política propia. Una reseña editorial podría crearse más adelante, de forma manual y visible, pero no es necesaria para el producto actual. G07 y G12 se centran en medir derivaciones, validar interés por asesorías y preparar una propuesta patrocinada transparente; Merchant Center queda fuera de las prioridades inmediatas.

### Actualización de ofertas y Jev — 25/09/2026

La verificación de extremo a extremo quedó documentada en [VALIDACION.md](VALIDACION.md): una oferta de CompraGamer se solicitó desde la API pública, se procesó mediante un despacho manual del workflow, se guardó en Supabase y apareció como oferta vigente y atribuida a la tienda en la ficha. Jev revisó su identidad textual con resultado consistente (0,81). Esta prueba de una oferta no representa cobertura general ni valida todavía la mejora de títulos originales en el barrido programado.

G02 sigue P0 y en observación. Las próximas tres acciones, en este orden, son: **(1)** medir y resolver la activación de la cola a pedido, porque el schedule observado tuvo intervalos cercanos a cinco horas y las solicitudes vencen a los 30 minutos; **(2)** registrar por ciclo y por tienda la cantidad de precios observados en tres horas, excluidos por identidad y realmente mostrados, junto con duración y consumo de Jev, antes de ajustar la cadencia del barrido; **(3)** revisar manualmente una muestra de coincidencias Jev de baja confianza, incluidas variantes tray/OEM y cooler, y comparar el próximo ciclo con los títulos originales antes de tocar el umbral de 0,8. La actualización a pedido es una ayuda puntual hasta que su disparador sea confiable; no se promete frescura universal del catálogo.

El corte medido a las 16:56 UTC encontró 64/47.727 ofertas disponibles observadas en ≤24 h; el [detalle por tienda](FRESCURA-TIENDAS-2026-09-25.csv) fija el denominador. Una ejecución nueva probó 80 observaciones persistidas en 11 tiendas, pero tres de los siete ciclos programados anteriores no obtuvieron productos. El workflow ya guarda un reporte por ciclo y falla ante cero observaciones. La vía de despacho inmediato está preparada, aunque no activa hasta configurar un token de GitHub limitado a este repositorio y comprobar una solicitud pública sin intervención manual. G02 no cumple todavía sus criterios de cierre; la [validación](VALIDACION.md) distingue cada prueba de las condiciones pendientes.

Actualización de las 17:12–17:18 UTC: el secreto limitado se configuró y una solicitud pública inició automáticamente [el workflow `36165637147`](https://github.com/john2k2/comparador-hardware-argentina/actions/runs/36165637147); la oferta solicitada se guardó como actualizada 29 segundos después de crear el job, sin intervención manual. La API confundió la respuesta exitosa HTTP 200 de GitHub con un fallo visual; `45cdc34` lo corrigió. Una segunda solicitud pública devolvió `dispatch=sent`, activó [el workflow `36166218562`](https://github.com/john2k2/comparador-hardware-argentina/actions/runs/36166218562) y terminó con la misma oferta actualizada y leída desde la base. El token vence el 25/10/2026. La activación puntual ya tiene evidencia, pero la cobertura/frescura de G02 sigue lejos de la meta y faltan siete ciclos diarios útiles medidos. Ver [VALIDACION.md](VALIDACION.md) para el alcance exacto.

### G02: muestra fija y fecha de comprobación — 25/09/2026

La [muestra prioritaria de nueve fichas](G02-MUESTRA-PRIORITARIA.json) fija tres CPU, tres GPU y tres RAM con ofertas disponibles. Es una muestra **operativa**, elegida para vigilar categorías centrales y variantes de memoria que necesitan separación de identidad; no representa estadísticamente todo el catálogo ni equivale a las páginas de mayor tráfico. El corte de las 18:06 UTC encontró **11/59 ofertas disponibles observadas en ≤24 h (18,6 %)** y **3/59 en ≤3 h**. La meta inicial propuesta de ≥95 % en 24 horas no está cumplida. El nuevo artefacto del cron agregará denominador y frescura de estas mismas nueve fichas en cada ciclo, además del detalle por tienda. No se reemplazan productos cuando una oferta deja de estar disponible: el denominador se informa cada vez.

Se movió la fecha objetivo de G02 al **03/10/2026** como primer control posible tras siete ciclos diarios nuevos (26/09–02/10); es una fecha de decisión, no una promesa de cierre. Cada ciclo cuenta solo con observaciones persistidas y un reporte válido. Si la muestra sigue bajo la meta, G02 permanecerá abierto y habrá que decidir cadencia/cobertura con duración, consumo de Jev y resultados por tienda medidos, sin elevar el gasto a ciegas. La prueba de despacho a pedido es una capacidad puntual, no reemplaza esa cobertura.
